...
function limpiar() {
    ...
    function cargarDatos(){
        ...
        db.transaction(function(transaction){
            var sql="SELECT *FROM comida ORDER BY id ASC";
            transaction.executeSql(sql,undefined,function(transaction,result){
                if(result.rows.length){
                    $("#listaProductos").append('<tr style= "background: #f9d423; "><th>Compra Nº</th><th>Producto</th><th>Cantidad</th><th>Precio</th><th>Total</th>');
    ...
    $("#insertar").click(function(){
        var nombre="Hamburguesa";
        var cantidad=$("#cantidad").val();
        var precio=5;
        var total=precio*cantidad;
        if(cantidad.length!=0&&cantidad>0){
        db.transaction(function(transaction){
            var sql="INSERT INTO comida(producto,cantidad,precio,total)VALUES(?,?,?,?)"
